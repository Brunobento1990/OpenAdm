...
For new DTO validation, inherit from `ValidarBaseDTO` and use the custom attributes in `OpenAdm.Application/Attributes`; do not use `System.ComponentModel.DataAnnotations`. Declare required fields through validation attributes, not through imperative checks in `Validar()`. Use `Validar()` overrides only for cross-field business rules, such as comparing an optional password and its confirmation; do not turn optional fields into required fields unless explicitly requested.

Before adding DTO- or service-local normalization helpers, check `OpenAdm.Domain/Extensions`. Reuse an existing extension when available; when the operation is generally reusable and missing, add it to the appropriate Domain extension class instead of creating private or internal static normalization methods in DTOs or services. New application configuration must be read through `IConfiguration` and stored in `appsettings`; `.env` is legacy and should not receive new settings.
